...
설치(위) 후 프로젝트 셋업은 자동이다 — 사람이 손으로 하는 단계가 없다.

- **신규 프로젝트**: 그냥 요청하면 된다. write-plan이 mode 판정 직후
  판정이 요구하는 전제(worktree 선언, 테스트 리소스 규칙)를 프로젝트
  CLAUDE.md에 **직접 기입**하고(프로비저닝 — 수정 파일은 PLAN.md·프로젝트
  CLAUDE.md 2개 한정 [엄격]), 기입 내역을 `프로비저닝: …` 1줄로 보고한다.
- **기존 프로젝트** (구버전 dev-kit 규격의 PLAN.md가 있는 경우):
  `/dev-kit:migrate` 1회 → 보고 확인 → "진행해"로 재개.
  mode 판정·기입, 프로비저닝, verify의 allowlist 형태 정리(검증 강도 변경
  금지 [엄격])까지 자동으로 수행하고, 위험 항목(부수효과 있는 verify,
  개발·공유 DB를 무는 테스트 설정, cross-role 의존 verify)은 수정하지 않고 목록으로 보고한다
  [엄격] — 해당 태스크는 사용자 결정 전까지 `hold:` 표시로 재개 대상에서
  제외된다. **멱등 [엄격]** — 재실행해도 변경 0건으로 통과한다.
...
/dev-kit:migrate               # 기존 프로젝트를 현행 규격으로 (멱등 — 위 "시작하기")
```
...
프로젝트 CLAUDE.md에 선언돼 있어야 켜진다 (없으면 `off`). 선언은 손으로
쓸 필요 없다 — mode A 판정 시 write-plan 프로비저닝(신규)·`/dev-kit:migrate`
(기존)가 직접 기입하고, 스택 판정이 불가하면 기입 대신 보고한다:
...
- **[엄격]** — 정확히 따른다. 맥락·급함을 이유로 완화하지 않는다:
  ...
  프로비저닝·migrate의 수정 파일 한정(각각 2개·3개), migrate의 멱등성·
  위험 항목 보고(수정 아닌 보고 + `hold:` 제외 표시)·검증 강도 변경 금지,
  mode 재판정의 B 상향 승인 게이트 유지.
...
### 프롬프트 캐시 적중률 보존 (v1.8.2)

프롬프트 캐싱은 Claude Code가 자동 적용하며 사용자가 조절할 수 없다. 다만
캐시는 접두사(에이전트 정의 + 도구 정의 + CLAUDE.md)가 동일할 때만 적중하므로,
루프 설계로 적중률을 관리할 수 있다. 캐시가 깨지면 서브에이전트 호출마다 고정
오버헤드를 정가로 재지불한다. 내장된 장치:

- **실행 중 CLAUDE.md 수정 금지** (execute-plan) — 헌법·프로젝트 CLAUDE.md는
  모든 서브에이전트 호출의 캐시 접두사다. 중간 수정은 남은 태스크 전부의
  캐시를 무효화하므로, 수정 태스크는 stage 말미로 미루고 수정 후 새 세션을
  권고한다.
- **CLAUDE.md 수정 태스크는 stage 말미 배치** (write-plan 분해 원칙 8,
  docs·harden 스킬 동일 적용).
- **같은 tier·risk 태스크 인접 정렬** (write-plan 분해 원칙 4) — 모델이
  바뀌면 캐시가 갈리므로, 같은 모델을 쓰는 태스크가 연속될수록 적중률이
  높아진다. 의존 순서가 최우선이고 동률일 때만 적용한다.
- **대기 시간 최소화**: 캐시 TTL은 짧다. 승인 대기로 루프가 멈추면 캐시가
  만료되어 다음 호출이 정가가 된다. verify에 쓰는 테스트·빌드 명령과 루프의
  git 명령을 permissions allow에 등록해 대기를 없애는 것은 안전 문제일 뿐
  아니라 비용 문제이기도 하다. (파괴적 명령은 v1.8 훅이 별도로 차단하므로
  allow 확대와 안전이 상충하지 않는다 — 아래 "권한 & 시크릿" 참조.)
...
- **[P]가 안 나오고 순차로만 계획됨** → PLAN.md의 verify들이 타 role 구현을
  ...
  검출은 grep으로 된다 — [P] 태스크 verify에 의존하는 계약 산출물 경로가
  명시돼야 하므로([P] verify 의존 명시 [엄격]), 경로 없이 외부에 의존하는
  verify가 [P]를 달고 있으면 규칙 위반이다.
