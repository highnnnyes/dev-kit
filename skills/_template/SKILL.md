...
- **SKILL.md 상단에 분류를 표기한다**: `[엄격]`(정확히 따름, 완화 금지) 또는
  `[유연]`(원칙 유지, 적용은 맥락 적응). 스킬 내부 세분 허용 —
  예: 전체는 [유연]이되 특정 안전선만 [엄격].
...
지시문은 상상이 아니라 관찰된 실패에서 나와야 한다:
1. **red** — 에이전트가 실제로 규칙을 어기거나 헛도는 사례 확인
   (PROGRESS.md FAIL 사유, 실사용 관찰).
2. **green** — 그 사례를 막는 최소한의 문장을 추가하고 행동이 바뀌는지 확인.
3. **refactor** — 중복·장황한 지시 정리, 행동 유지 확인.

관찰된 실패 없이 "있으면 좋을 것 같은" 규칙을 추가하지 마라 —
매 로드마다 토큰을 쓰면서 아무 행동도 바꾸지 않는 문장이 된다.

green이 잘 안 나오면 규칙을 서술형 지시가 아니라 **산출물 제약**으로 바꿔
써라 (헌법 §1 "규율은 산출물로 강제한다") — 안 지켰을 때 출력에 빈칸으로
드러나는 형태가 아니면 그 규칙은 강제되지 않는다.
