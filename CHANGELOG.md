...
docs/screens.md만 보고 답할 수 있어야 하고, "적혀 있지 않음"이 곧 지적이다.

- **brainstorming**: 시나리오 걷기 직후·저장 전 휴리스틱 점검 [엄격] —
  걸리는 항목은 저장 전에 screens.md 반영 또는 사용자 확인. HTML 목업
  재생성은 기존 규칙(확정 후 1회) 유지.
- **grill**: 대상 파악에 screens.md 병독, 화면/플로우 각도에 동일 5종
  추가(자기 점검을 통과한 문서 재심문이므로 체크리스트 재확인이 아니라
  점검이 놓쳤을 법한 화면을 골라 묻는다 — 세션당 5~10개 질문 원칙 유지),
  산출물에 screens.md 갱신 포함.
- audit C에 두 파일 휴리스틱 목록 일치 검사 1항목. 이 외 수정 없음.

## v1.14.0 — 2026-09-12
화면 설계 단계 신설 + metrics 정직성·해상도 수정 + README 실측 첫 기재.
원칙: "쓰기 불편함"은 verify 명령 기반 검증 사슬의 어느 층에서도 안 잡힌다 —
화면 스펙을 상류(설계)에서 확정해 verify·추적표·drift 검사에 관측 가능하게
건다.

- **화면 설계 단계 (UI가 있는 기능 한정)**: brainstorming 4단계에
  사용자/화면/플로우 섹션 추가(UI 프로젝트 최소 안전선에 "화면 목록 확정"
  포함 [엄격]) — 화면당 확정 항목(목적·진입 경로·액션과 결과 피드백·입력
  필드·상태 3종·이탈/완료 동작), 저장 전 시나리오 걷기 [엄격](핵심
  시나리오별 정상+실패 경로 완주 확인, 5개 초과는 설계 분해), 산출물
  `docs/screens.md`(3화면 이상은 builder-light 위임 로파이 목업
  `docs/screens/index.html`, 2개 이하 ASCII만 [유연]). 연동: grill
  화면/플로우 심문 각도, write-plan(screens.md 입력 + role=frontend verify에
  `docs/screens.md#화면명 기준` 명시 [엄격] — 없는 UI 태스크는 DECISIONS
  정지 + 화면 변경 stage 말미 screens.md 갱신 태스크), reviewer 추적표
  화면 행(기준은 브리핑의 화면 섹션 발췌 — 리뷰 입력에 조건부 ⑤ 신설,
  execute-plan 브리핑 규칙 동시 수정. 증거는 테스트 또는 **이미 존재하는**
  `npx playwright screenshot` 이미지 Read만 — 생성 명령 직접 실행 금지),
  stage-reviewer drift 대상·docs 5종 등재(기존 절차형 verify "drift 스캔
  통과" 범위 안 — 헌법 개정 불필요), 헌법 §4는 1줄만 추가.
- **/dev-kit:metrics 수정**: 출력 첫 행 "입력 파일" 고정 [엄격] — 실제
  파싱한 파일 경로+블록 수, 못 연 파일 `없음`, 템플릿 문구 전사 금지
  (실측에서 archive를 읽지 않고 읽었다고 출력한 버그의 봉합 — 위반이
  산출물에서 드러나게). archive 탐색 규칙 명시(같은 디렉토리). 소요를
  중앙값 3분리(시도 1회 한정·tier=light 한정·300분 초과 이상치 제외,
  각 n 표기 + 해석 규칙 1줄). red 행에 `- red:` 줄 부재 건수 별도 표기
  (미확인 FAIL과 기록 누락은 다른 현상).
- **README "Measured on real runs" 첫 기재**: myroquant 9월분(116태스크)
  수정된 metrics 출력 원문 — 재시도율 30.2%·병렬 효율 1.01×·red 78%.
  단일 프로젝트 스냅샷 명시(순차 vs mode A 비교는 병렬 효율 1.0× 초과 런
  이후). 아키텍처 다이어그램·구성 요소 표에 screens.md 단계 반영.

## v1.13.1 — 2026-08-21
v1.13.0 자기 평가에서 나온 강제 구멍 2건 봉합. 원칙: 헌법 §1 — [엄격]
규칙은 어겼을 때 산출물에 빈칸으로 드러나야 강제된다. v1.13.0의 cross-role
금지는 지시문 형태였고(플래너가 회귀해도 PLAN.md에서 기계 검출 불가),
기존 계획의 위반은 어떤 층에서도 안 잡혔다.

- **[P] verify 의존 명시 [엄격]** (write-plan 계약 스테이지 신설): [P]
  태스크의 verify가 자기 파일 밖에 의존하면 그 대상을 Stage 0 계약 산출물
  경로로 verify 문구에 명시한다 (예: `mocks/api.ts 대상`). 경로로 명시할
  수 없는 외부 의존(실 서버·타 role 구현 실행)은 [P] 부여 금지. 외부 의존
  없는 verify는 대상 아님(픽스처류 [P] 유지). → 위반이 verify 문구의
  grep 가능한 빈칸으로 관측된다. PLAN.md 예시 1.2를 경로 명시 형태로 구체화.
- **migrate 위험 항목 보고에 cross-role 의존 verify 추가**: 수정 없이
  보고 + `hold:` 표시(검증 강도 변경 금지와 정합 — 교체가 아니라 보고),
  분리안("mock 대상 verify + 통합 stage 태스크")을 보고에 함께 제시 —
  v1.13 이전 PLAN.md의 위반을 잡는 이행 경로.
- 단일 소스 파생 예시에 "공유 타입 파일" 추가(스키마 도구 없는 스택 대응),
  audit C 검사 보강(명시 규칙 write-plan·README 존재 + migrate 포함 여부),
  README(구성 요소 표 write-plan·migrate 행, 시작하기, 튜닝 가이드) 반영.
  헌법 §4는 무수정(요약 범위 밖 — 상세 규칙은 write-plan 소관).
...
수동 프롬프트로 안내되던 절차(mode 기입·worktree 선언·verify 정리·테스트
리소스 규칙)를 전부 흡수한다.

- **프로비저닝 단계** (write-plan [엄격]): mode 판정 직후 판정이 요구하는
  전제를 직접 기입 — 사용자 안내 금지, 수행이 목적. mode A면 worktree
  선언(값은 스택으로 판정, 불가하면 기입 대신 보고 — off 유지)·테스트
  리소스 규칙(테스트 전용 DB만, 개발·공유 DB 금지)을 프로젝트 CLAUDE.md에
  추가. mode S는 추가하지 않음(권한 최소화 — 있으면 제거도 안 함), B는
  프로비저닝 대상 아님(승인 절차 유지). 수정 파일 PLAN.md·프로젝트
  CLAUDE.md 2개 한정, `프로비저닝: …` 1줄 보고(없어도 "없음" 명기).
- **mode 재판정** (write-plan·execute-plan): 트리거 2개 — ① 계획 수정 시
  (헤더 `mode: A (재판정: 날짜, 이전 S, 근거)` 갱신 + 프로비저닝 재수행),
  ② stop-the-world(중단 조건 (i)) 후 계획 수정 시 재개 전 강제. 규칙
  [엄격]: S→A 상향 자동, A→S 하향은 진행 중 [P] 없을 때만(있으면 현재
  그룹 완료 후 전환 — 실행 중 스케줄링 모델 변경 방지), B 상향은
  재판정에서도 승인 게이트 유지. 재판정 프로비저닝의 CLAUDE.md 기입은
  캐시 접두사를 가르므로 새 세션 재개 권고.
- **`/dev-kit:migrate` 신설** (commands/migrate.md): 기존 프로젝트 → 현행
  규격 단일 명령. ① mode 판정·기입(`오버라이드: migrate` — B는 어떤
  경우에도 자동 기입 금지, A 기입 + "B 후보" 보고) → ② 프로비저닝 →
  ③ verify 정리(allowlist 형태로 교체, 필요 시 package.json scripts 추가 —
  **검증 강도 변경 금지 [엄격]**, 절차형 2종 유지) → ④ **위험 항목 보고
  [엄격]**(부수효과 verify·개발/공유 DB 테스트 설정은 수정 없이 목록 보고 +
  태스크에 `hold:` 표시 — execute-plan이 건너뛰고 보고에 포함, 사용자
  결정 시 복귀) → ⑤ 자기 검사(수정 파일 3개 한정 [엄격] — 벗어나면 커밋
  없이 중단) 후 `[dev-kit-migrate] v<버전> 규격 반영` 단일 커밋.
  **멱등 [엄격]** — 재실행 시 변경 0건 통과.
- README: "시작하기 — 2경로"(신규 = 그냥 write-plan, 기존 = migrate 1회 →
  보고 확인 → 재개), 수동 커맨드·구성 요소 표에 migrate, [엄격] 목록에
  파일 한정·멱등성·위험 보고 추가. 워크트리 선언 안내를 자동 기입 기준으로
  갱신(수동 추가 서술 제거).
